...
@section('content')
    <div class="bg-gradient-to-r from-primary-500 to-blue-600 text-white py-16">
        <div class="container mx-auto px-4 text-center">
            <h1 class="text-5xl font-extrabold mb-4">{{ __('Product Categories') }}</h1>
            <p class="text-xl text-blue-100">{{ __('Explore our complete range of chemical products') }}</p>
        </div>
    </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            @foreach($categories as $category)
                <a href="{{ route('categories.show', $category->slug) }}" class="group">
                    <div class="bg-white rounded-2xl shadow-lg p-10 hover-lift border-2 border-transparent hover:border-primary-500 transition-all">
